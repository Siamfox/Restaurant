Function Convnum(Total)
  ...
  Do case
    ...
    Otherwise
      ...
      Texto = strtran(tMilMillones + tMillones + tMiles + tCientos, '  ', ' ') + ' BOLIVARES'
  Endcase

Return Texto + iif(!empty(Total), ' CON ' + strtran(transform(int((total - int(total)) * 100), '**'), '*', '0') + '/100', '')
