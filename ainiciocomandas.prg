...
*---------------------------------------------------
*--- Enrutar Almacenamiento,  Programas  y Librerias
*---------------------------------------------------
*!*	druta=SYS(5)+CURDIR()  
*!*	pruta=SYS(5)+CURDIR()			
...
*!*	druta=LOWER('\\LUISP\SiamResto\')			&&+SYS(5)+CURDIR()  
*!*	pruta=LOWER('\\LUISP\SiamResto\')			&&+SYS(5)+CURDIR()	
...
IF EMPTY(xsta) .or. xsta='1'
	...
	druta=alltrim('\SiamResto\')			&&+SYS(5)+CURDIR()  
	pruta=alltrim('\SiamResto\')			&&+SYS(5)+CURDIR()			

ELSE

	druta=ALLTRIM('\\Caja\SiamResto\')			&&+SYS(5)+CURDIR()  
	pruta=ALLTRIM('\\Caja\SiamResto\')			&&+SYS(5)+CURDIR()			
...
*!*		SIAMEMO()
*!*		SIAMEMOLOC(xsta)
*!*		**WAIT WINDOWS 'Iniciando Estacion Principal...' NOWAIT &&TIMEOUT 0.5
*!*	ELSE
	xsta=sta
	SIAMEMO()
	SIAMEMOLOC(xsta)
	**WAIT WINDOWS 'Iniciando Estacion '+xsta+' Multipuntos...' NOWAIT &&TIMEOUT 0.5 
*!*	ENDIF
...
**--
 IF !IS_RUN('comanderas.exe')  && QUE NO SE CARGE 2 VECES EL SISTEMA
...
iF NOT F_ActivaWin("bienvenida_comandas")
	*-------------------------------------------
	**--- Activar Menu Principal
	*-------------------------------------------	
	DO FORM bienvenida_comandas
...
*---Verificar Licencia
*---------------------------------------------
**WAIT WINDOWS 'Verificando Activacion del Sistema, Por favor espere...' NOWAIT &&TIMEOUT 0.5 
licencia()
...
*---------------------------------------------
**WAIT WINDOWS 'Verificando Tablas del Sistema, Por favor espere...' NOWAIT &&TIMEOUT 0.5 
***buscarTablas()
