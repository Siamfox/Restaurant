...
IF EOF()
   ...
   RETURN
ELSE
	STORE FECHA 	TO FECHA2   
	STORE SERIALFIS	TO WSERIALFISCAL
	STORE WNRO		TO WFAC2
ENDIF
...
**---------------------------------------------------------------
**// OTROS VALORES PARA LA NOTA DE CREDITO
**---------------------------------------------------------------
...
**---------------------------------------------------------------
   CHOR =SUBSTR(TTOC(dateTIME(),2),1,2)
   CMIN =SUBSTR(TTOC(dateTIME(),2),4,2)
   CSEG =SUBSTR(TTOC(dateTIME(),2),7,2)

**---------------------------------------------------------------
**// CONTROL OPERACIONES
**---------------------------------------------------------------
   WCOO='12345' &&FCOO  && NUMERO CONTROL (COO) TRANSACCION && '12345'
   IF WCOO=""
      WAIT WINDOWS '!!! ERROR !!! , (MAESTRO NOTAS)->SIN NRO. CTROL (COO) FISCAL '
      RETURN
   ENDIF
...
**---------------------------------------------------------------
   *WSERIE=CSERIE && '1FB8100000'
   IF WSERIALFISCAL=SPACE(15)
      WAIT WINDOWS '!!! ERROR !!! ,(MAESTRO MEMO)->SIN NRO. SERIAL IMPRESORA FISCAL '
      RETURN
   ENDIF
...
@ L,0 SAY '276|'+CCLIENTE+'|'+WSERIALFISCAL+'|'+CRIF+;
             '|'+CDIA+'|'+CMES+'|'+CANO+'|'+CHOR+;
             '|'+CMIN+'|'+CSEG+'|'+STR(WFAC2)+'|%'
L=l+1
...
@ L,0 SAY '040|'+'D'+'|'+'$'+'|'+'0000'+'|%'
L=l+1
@ L,0 SAY '082|'+'(COO)# '+WCOO+' !!!... Gracias por su Compra...!!!'+'|%'
