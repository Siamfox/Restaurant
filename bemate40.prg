...
CASE MAYDET=.F. 	&& Detal
	maestro='DET'+ALLTRIM(XSTA)
	registros='DREG'+ALLTRIM(XSTA)
	IND_I='DET'+ALLTRIM(XSTA)+'_NRO'
	IND_II='DREG'+ALLTRIM(XSTA)+'_NRO'
		
		
CASE MAYDET=.T.		&& Mayor
	maestro='FAC'+ALLTRIM(XSTA)
	registros='FREG'+ALLTRIM(XSTA)
	IND_I='FAC'+ALLTRIM(XSTA)+'_NRO'
	IND_II='FREG'+ALLTRIM(XSTA)+'_NRO'
...
DO WHILE !EOF()
	...
	**//Venta Articulo
	**---------------------------------------------------------------
	iRetorno = Bematech_FI_VendeArticulo( ALLTRIM(fCodigo), ALLTRIM(SUBSTR(fDescrip,1,4)), ALLTRIM(falicuota), ALLTRIM(funifra), ALLTRIM(fcantidad), fcant_dec, ALLTRIM(fprecio), ALLTRIM(ftipo_des), ALLTRIM(fdescuento) )


	IF iRetorno<>1
		DO CASE 
			CASE iRetorno= 0
				WAIT WINDOWS 'Error de comunicación' 
				return
			*CASE iRetorno=1
				**WAIT WINDOWS 'OK' NOWAIT 
			CASE iRetorno=-2
				WAIT WINDOWS '-2: Parámetro inválido en la función'
				return
			CASE iRetorno=-3
				WAIT WINDOWS 'Alícuota no programada'
				return
			CASE iRetorno=-4
				WAIT WINDOWS '-4: El archivo de inicialización BemaFI32.ini no fue encontrado en el directorio de sistema del Windows' 
				return
			CASE iRetorno=-5
				WAIT WINDOWS '-5: Error al abrir la puerto de comunicación'
				return
		ENDCASE
	ENDIF
