...
SET OPTIMIZE ON 
SELECT nro,fecha,tpago,monto,banco,detalle,serie FROM FORCE cajapagos WHERE wfactura=Nro INTO CURSOR &temp_caj
SET OPTIMIZE OFF 
...
*!*					CHR(13)+'Pulse Enter, para continuar...',0+64,'Atencion')
	...
	**--Actualizo el precio sin iva del 12%
	UPDATE detreg SET precio=(precio/(1+alicuota*.01)) WHERE wfactura=Nro .and. talicuota=1 

	**--Actualizo el precio nuevo con el iva nuevo 10%
	UPDATE detreg SET alicuota=10, talicuota=4, precio=(precio*(1+alicuota*.01)) WHERE wfactura=Nro .and. talicuota=1 
